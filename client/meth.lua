...
local function ProcessChemicals()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('mrich-jobpack:processChemicals')
...
local function ProcessTempUp()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.temp_up"), 15000, false, true, {
		...
		TriggerServerEvent('mrich-jobpack:processTempUp')
...
local function ProcessTempDown()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.temp_down"), 15000, false, true, {
		...
		TriggerServerEvent('mrich-jobpack:processTempDown')
...
local function ProcessProduct()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.packing"), 15000, false, true, {
		...
		TriggerServerEvent('mrich-jobpack:processMeth')
...
RegisterNetEvent('mrich-jobpack:ProcessChemicals', function()
...
RegisterNetEvent('mrich-jobpack:ChangeTemp', function()
			...
			if result.ret then
				exports['ps-ui']:Thermite(function(success)
					if success then
						...
						TriggerServerEvent('mrich-jobpack:cancelProcessing')
						TriggerServerEvent('mrich-jobpack:processFailUp')
					end
				end, 10, 5, 3)
			else
				QBCore.Functions.Notify(Lang:t("error.no_item", {item = result.item}))
...
RegisterNetEvent('mrich-jobpack:ChangeTemp2', function()
			...
			if result.ret then
				exports['ps-ui']:Thermite(function(success)
					if success then
						...
						TriggerServerEvent('mrich-jobpack:cancelProcessing')
						TriggerServerEvent('mrich-jobpack:processFailDown')
					end
				end, 10, 5, 3)
			else
				QBCore.Functions.Notify(Lang:t("error.no_item", {item = result.item}))
...
RegisterNetEvent('mrich-jobpack:ProcessProduct', function()
	local coords = GetEntityCoords(PlayerPedId(source))
...
RegisterNetEvent('mrich-jobpack:EnterLab', function()
	local ped = PlayerPedId()
...
RegisterNetEvent('mrich-jobpack:ExitLab', function()
	local ped = PlayerPedId()
	local pos = GetEntityCoords(ped)
